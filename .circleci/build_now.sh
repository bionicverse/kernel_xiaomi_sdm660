...
then
	...
	then
		...
		export KBUILD_BUILD_HOST="Bionic"
...
#Now Its time for other stuffs like cloning, exporting, etc
 ...
 clone() {
	...
	if [ $COMPILER = "clang" ]
	then
		msg "|| Cloning Bionic Clang ||"
		git clone --depth=1 https://github.com/bionicverse/bionic-clang clang-llvm
...
build_kernel() {
	...
	make -j"$PROCS" O=out \
		...
		if [ "$PTTG" = 1 ]
 			then
				tg_post_build "compile.log" "<b>Build failed to compile after $((DIFF / 60)) minute(s) and $((DIFF % 60)) seconds</b>"
			fi
		fi
...
if [ $LOG_DEBUG = "1" ]
then
	tg_post_build "compile.log" "$CHAT_ID" "Debug Mode Logs"
fi
